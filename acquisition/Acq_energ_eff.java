...
public class Acquisition {
	...
	public boolean startAcquisition() {
		...
		float sumreal = 0;
		float sumimag = 0;
		final float twoPIdivbyFs = 2 * lFPI/FREQ_SAMPLING;
		...
		{
			int fd = FREQ_MOD_MIN + FREQ_MOD_STEP * j;
			// --- For each fd, compute Xin[n]*e^(-2j*pi*fd*n/fs)
			for (int i = 0; i < sCount; i=i+1)
			{
				float cos = PETrigonometry.cos(twoPIdivbyFs * fd * i );
				float sin = PETrigonometry.sin(twoPIdivbyFs * fd * i );
				res_r_1[i] =  sample_r[i] * cos + sample_i[i] * sin;
				res_i_1[i] = -sample_r[i] * sin + sample_i[i] * cos;
			}
